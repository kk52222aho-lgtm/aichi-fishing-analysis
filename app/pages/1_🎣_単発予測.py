...
st.info(
    "ℹ️ **精度は限定的・相対的な参考としてご利用ください。** "
    "検証の結果、出船前情報から「絶対の匹数」や「ピタリの調子」を当てることはできません"
    "（釣果は当日の運・要因に大きく左右されるため）。一方で『**いつもより良い／悪い**』という"
    "**相対的な傾向**は、平年だけの予測よりわずかに当たる弱い精度があります（順位相関 0.1〜0.3 程度）。"
    "数字そのものより**相対傾向の参考**に。確実な季節傾向は **📅 釣果カレンダー** を参照してください。"
)
